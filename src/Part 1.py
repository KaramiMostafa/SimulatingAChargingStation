...
def mser_method(smoothed_metric, smoothed_time):
    ...
    for d in range(int(len(smoothed_metric) - 1)):
        ...
        mser.append(
            sum([(x - avg_trunc_metric) ** 2 for x in trunc_metric_list])
            / (len(trunc_metric_list) ** 2)
        )
...
def randomization_test(smoothed_metric, smoothed_time):
    ...
    avg_metric_batchmeans = [
        sum(smoothed_metric[x : min(len(smoothed_metric), (x + N))])
        / len(smoothed_metric[x : min(len(smoothed_metric), (x + N))])
        for x in range(0, len(smoothed_metric), N)
    ]
...
plot_over_time(
    metric=avg_waiting_delay,
    title="Average Waiting Delay of EVs from Simulation time = 0",
    ylabel="Avg. Waiting Delay",
)
...
cutoff_idx = next(
    x
    for x, val in enumerate([x["time"] for x in avg_waiting_delay])
    if val > transient_cutoff_time
)
point_estimate_mean = sum(
    [x["avg_wait_delay"] for x in avg_waiting_delay][cutoff_idx:]
) / len(avg_waiting_delay[cutoff_idx:])
...
for N_runs in runs:
    ...
    print(
        "CI Width: "
        + str(
            confidence_interval(avg_waiting_delay_list)[2]
            - confidence_interval(avg_waiting_delay_list)[1]
        )
    )
    print("")
