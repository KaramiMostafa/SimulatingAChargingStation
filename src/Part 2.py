...
def confidence_interval(avg_waiting_delay_list, colname, transient_cutoff_time):
    ...
    for i in range(len(metric)):
        cutoff_idx = [
            x for x, val in enumerate(time[i]) if val > transient_cutoff_time
        ][0]
...
smoothed_metric, smoothed_time = fishmans_method(
    EV_waiting_delay_list, "wait_delay", "Wait Time", transient_cutoff_time
)
smoothed_metric, smoothed_time = fishmans_method(
    EV_avg_waiting_delay_list,
    "avg_wait_delay",
    "Average Wait Time",
    transient_cutoff_time,
)
smoothed_metric, smoothed_time = fishmans_method(
    EV_missed_service_prob_list,
    "prob",
    "Missed Service Probability - EV",
    transient_cutoff_time,
)
smoothed_metric, smoothed_time = fishmans_method(
    EV_queue_length_list, "qlength", "EV Queue Length", transient_cutoff_time
)
smoothed_metric, smoothed_time = fishmans_method(
    avg_EV_queue_length_list,
    "avg_qlength",
    "EV Average Queue Length",
    transient_cutoff_time,
)
smoothed_metric, smoothed_time = fishmans_method(
    charge_queue_length_list, "qlength", "Charge Queue Length", transient_cutoff_time
)
smoothed_metric, smoothed_time = fishmans_method(
    avg_charge_queue_length_list,
    "avg_qlength",
    "Average Charge Queue Length",
    transient_cutoff_time,
)
smoothed_metric, smoothed_time = fishmans_method(
    standby_queue_length_list, "qlength", "Standy Queue Length", transient_cutoff_time
)
smoothed_metric, smoothed_time = fishmans_method(
    avg_standby_queue_length_list,
    "avg_qlength",
    "Average Standy Queue Length",
    transient_cutoff_time,
)
...
print(
    ...
    + str(
        confidence_interval(
            EV_avg_waiting_delay_list, "avg_wait_delay", transient_cutoff_time
        )
    )
)
print(
    "EV Missed Service Probability: "
    + str(
        confidence_interval(EV_missed_service_prob_list, "prob", transient_cutoff_time)
    )
)
print(
    "Average EV Queue Length: "
    + str(
        confidence_interval(
            avg_EV_queue_length_list, "avg_qlength", transient_cutoff_time
        )
    )
)
print(
    ...
    + str(
        confidence_interval(
            avg_charge_queue_length_list, "avg_qlength", transient_cutoff_time
        )
    )
)
print(
    "Average Standby Queue Length: "
    + str(
        confidence_interval(
            avg_standby_queue_length_list, "avg_qlength", transient_cutoff_time
        )
    )
)
