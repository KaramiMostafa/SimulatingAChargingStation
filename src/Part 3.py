...
window = [
    ...
    22,
    23,
]  # Postponing of charging for a fraction of batteries during this period
...
def arrival(EV_id, time, FES, charge_queue, EV_queue, standby_queue):
    ...
    data.avg_EV_queue_length.append(
        {
            "time": time,
            "avg_qlength": sum([x["qlength"] for x in data.EV_queue_length])
            / len(data.EV_queue_length),
        }
    )

    data.EV_missed_service_prob.append(
        {"time": time, "prob": round(data.EV_reneged / data.EVarr, 2)}
    )
    ...
    if bat_cnt_in_standby > 0:
        ...
        data.EV_waiting_delay.append(
            {"time": time, "wait_delay": time - EV.arrival_time}
        )
        data.EV_avg_waiting_delay.append(
            {
                "time": time,
                "avg_wait_delay": sum([x["wait_delay"] for x in data.EV_waiting_delay])
                / len(data.EV_waiting_delay),
            }
        )

        data.charge_queue_length.append({"time": time, "qlength": len(charge_queue)})
        data.avg_charge_queue_length.append(
            {
                "time": time,
                "avg_qlength": sum([x["qlength"] for x in data.charge_queue_length])
                / len(data.charge_queue_length),
            }
        )

        data.standby_queue_length.append({"time": time, "qlength": len(standby_queue)})
        data.avg_standby_queue_length.append(
            {
                "time": time,
                "avg_qlength": sum([x["qlength"] for x in data.standby_queue_length])
                / len(data.standby_queue_length),
            }
        )
        ...
        if bat_cnt_charging < N_BSS:
            ...
            if random.choices([1, 0], [f, 1 - f])[0] == 1 and (
                int(time) % 24 in window
            ):
                ...
                availability_alert_time = round(
                    (B_TH - initial_charge) / charging_rate, 2
                )
                ...
                data.charge_queue_length.append(
                    {"time": time, "qlength": len(charge_queue)}
                )
                data.avg_charge_queue_length.append(
                    {
                        "time": time,
                        "avg_qlength": sum(
                            [x["qlength"] for x in data.charge_queue_length]
                        )
                        / len(data.charge_queue_length),
                    }
                )
            ...
            data.EV_avg_waiting_delay.append(
                {
                    "time": time,
                    "avg_wait_delay": sum(
                        [x["wait_delay"] for x in data.EV_waiting_delay]
                    )
                    / len(data.EV_waiting_delay),
                }
            )
...
def main_simulation(RANDOM_SEED):
    ...
    while time < SIM_TIME:
        ...
        # For other events, call the corresponding event handler function
            ...
            event_type == "Availability Alert"
            and int(time % 24) in partial_charge_times
        ):
...
def get_electricity_prices(filename="electricity_prices.csv"):
    ...
    electricity_prices = spring_prices.append(
        [summer_prices, fall_prices, winter_prices]
    ).reset_index(drop=True)
    electricity_prices["Season"] = electricity_prices["Season"].apply(
        lambda x: x.replace(":", "")
    )
...
def get_PV_capacity(filename="PVproduction_PanelSize1kWp.csv"):
    ...
    production_by_season = pv_production.groupby(["Season", "Hour"]).agg(
        {"Output power (W)": "mean"}
    )
...
def get_cost(util_by_hour, season, S_PV=100, N_BSS=2):
    ...
    production_summer = production_by_season.loc[
        production_by_season["Season"] == "Summer", "Output power (W)"
    ].tolist()
    production_winter = production_by_season.loc[
        production_by_season["Season"] == "Winter", "Output power (W)"
    ].tolist()
    ...
    if season == "Summer":
        power_from_grid = [
            max(x - y, 0) for x, y in zip(avg_power_consumed, production_summer)
        ]
        prices_summer = electricity_prices.loc[
            electricity_prices["Season"] == "SUMMER", "Price"
        ].tolist()
        cost = sum([x * y / 1000 for x, y in zip(power_from_grid, prices_summer)])

    elif season == "Winter":
        power_from_grid = [
            max(x - y, 0) for x, y in zip(avg_power_consumed, production_winter)
        ]
        prices_winter = electricity_prices.loc[
            electricity_prices["Season"] == "WINTER", "Price"
        ].tolist()
    ...
    for i in range(N_runs):
        ...
        for bat in recharged_bats:
            ...
            if bat in list(data.recharge_end_times.keys()):
                recharge_time = (
                    data.recharge_end_times[bat] - data.recharge_start_times[bat]
                )
            else:
                recharge_time = SIM_TIME - data.recharge_start_times[bat]

            temp_time = (recharge_start_time - int(recharge_start_time)) + recharge_time
            temp_time_list = [1 for x in range(int(temp_time))] + [
                round(temp_time - int(temp_time), 2)
            ]
            temp_time_list[0] = round(
                temp_time_list[0] - (recharge_start_time - int(recharge_start_time)), 2
            )
            for j in range(
                int(recharge_start_time),
                min(int(recharge_start_time) + len(temp_time_list), SIM_TIME),
            ):
                temp_util_list[j] = (
                    temp_util_list[j] + temp_time_list[j - int(recharge_start_time)]
                )

        temp_util_list_24 = []
        for j in range(24):
            temp_util_list_24.append(
                sum([temp_util_list[x] for x in range(j, len(temp_util_list), 24)])
                / len([temp_util_list[x] for x in range(j, len(temp_util_list), 24)])
            )
